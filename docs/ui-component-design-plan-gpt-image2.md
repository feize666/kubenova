...
| Filters | Scope popover, network filters, table global search, table filter row, topology filter pills. | Define one filter grammar: global keyword, cluster/namespace scope, domain facets, table columns. Same active-count badge semantics. |
| Menus and row actions | `ResourceActionDropdown`, `ResourceRowActions`, page-local actions, confirm modal integration. | Use one compact trigger style. Keep destructive actions in danger section with confirm copy. |
| Drawers | Resource detail, YAML edit, business detail, cluster detail. Width and content structures differ. | Define drawer classes: resource detail, editor drawer, business detail, workbench drawer. Align width, footer/header action placement, loading/error state. |
...
| Tags and status | `StatusTag` exists but many direct AntD `Tag` colors and inline colors remain. | Move to semantic states: success, warning, danger, info, neutral, unknown. Avoid arbitrary color names at route level. |
