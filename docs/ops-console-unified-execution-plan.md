...
5. Deliver cluster domain, worker nodes, network resources, global detail drawers, panorama, observability, and KubeNova.
...
- Observability center foundation slice:
  ...
  - `git diff --check`: passed.
- KubeNova center foundation slice:
  ...
  - `KubeNova中台` navigation now exposes `事故中台` and `AI 助手`.
  - `docs/kubenova-regression-matrix.md` added for Requirement 10 coverage across anomaly overview, incident queue, correlation groups, root-cause candidates, recommendation, precheck, approval, audit, and rollback hints.
...
- Performance and stability hardening slice:
  ...
  - KubeNova summary validates inverted `from`/`to` ranges before service execution.
  ...
  - Backend smoke: table preference GET/PUT returned 200, observability summary returned 200 in about 2.46s, KubeNova inverted range returned 400 `REQUEST_FAILED`.
  - Pressure smoke: 20 authenticated backend requests at concurrency 5 across capabilities, preferences, observability, and KubeNova returned all 200 with no 5xx; p95 about 2554ms.
  - `cd backend/control-api && npm run test -- aiops.controller.spec.ts monitoring.controller.spec.ts --runInBand`: passed, 2 suites / 7 tests.
...
- Release gate focused regression:
  ...
  - 12.7 KubeNova scoped browser regression covered `/aiops` summary, degraded analysis banner, Incident Workbench open, recommendation expansion, Precheck, and approval actions with `consoleErrorCount=0`.
  - 12.7 fixes added contextual Ant Design message usage on KubeNova and cluster-health action paths, removed deprecated Ant Design `List` from Incident Workbench, and deduplicated derived KubeNova incident ids before rows/recommendations/root-cause candidates are returned.
  ...
  - 12.9 backend pressure smoke covered 20 authenticated requests at concurrency 5 across capabilities, table preferences, observability summary, and KubeNova summary; result `20/20` status 200, no 5xx, `p50Ms=9`, `p95Ms=2531`, `maxMs=2532`.
  - 12.9 KubeNova error-path smoke confirmed inverted `from`/`to` returned `400 REQUEST_FAILED` with message `` `from` 不能晚于 `to` ``.
...
- Open task audit (2026-05-30):
  ...
  - 0.2 can be marked complete from current evidence. Theme tokens, shell theme toggle, navigation IA, dashboard/KubeNova pages, and pod metric degraded semantics exist; full visual QA remains 1.3/12.2 follow-up.
  ...
  - 0.5 can be marked complete from current evidence. Observability and KubeNova regression matrices, summary APIs, permission-gated nav, approval/audit flows, source degradation, and external deep-link fields are documented and tested.
