...
- UI audit slice:
  ...
  - Static evidence also confirmed remaining 1.2 gaps: many page-local `Modal` / `Modal.confirm` flows, mixed direct `Tag` colors, pod-specific action styling, local AntD `Table` exceptions, drawer-family width differences, and many inline `rgba(...)`/hex/gradient colors.
