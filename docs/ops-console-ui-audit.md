...
| --- | --- | --- | --- |
| Shared buttons / row actions | `ResourceAddButton` centralizes create button tooltip/shape; `ResourceActionDropdown` and `renderResourceActionTriggerButton` centralize compact row actions with `aria-label`; `ResourceActionBar` inline actions now render through `OpsIconActionButton`; `ResourceTableToolbar` uses named search/filter/clear/column buttons. `OpsPopoverPanel` footer reset/apply actions now use `OpsIconActionButton`. Shell topbar notification/theme controls now use `OpsIconActionButton`. Users and RBAC table toolbar extras now use `ResourceAddButton` plus `OpsIconActionButton` for create/refresh. Topology zoom/detail/retry/breadcrumb/quick-action/resource-tree toolbar buttons and AI assistant action/quick-prompt/diagnosis buttons now use `OpsIconActionButton`. Pod pages still carry pod-specific action CSS variables and some pages compose local buttons. | Shared button primitives exist and cover common create/table/action entry points. This closes the static existence part of 1.2 for shared button foundations. | Visual consistency, hit target, focus ring, disabled/loading state, and popover/dropdown keyboard behavior across representative rows still need browser pass. |
...
| Status chips / badges | `OpsStatusTag`, `OpsFilterChip`, `StatusTag`, and resource-detail `DetailChipList` now cover shared status/chip display. Static scan shows no remaining AntD `Tag`, `.ant-tag`, `<Tag>`, or `TagList` usage under `frontend/src`; route-level chip colors now pass through semantic ops tones. | Static tag-normalization gap can be marked closed. | Need visual contrast review in both themes and mobile truncation for long Kubernetes labels and status chips. |
| Filters | `ResourceFilterToolbar`, `ResourceClusterNamespaceFilters`, `NetworkResourcePageFilters`, `ResourceScopeFilterButton`, facet buttons, table filter row, and URL-sync hooks exist. `OpsFilterTriggerButton` now backs resource scope/facet triggers and topology toolbar pills, while preserving page-specific slot classes. CSS includes responsive toolbar rules at `960px` and table toolbar rules at `720px`. | Static evidence closes the existence of shared filter primitives and responsive CSS hooks. | Still open: route-family consistency between global search, cluster/namespace filters, table column filters, topology popovers, and URL state needs browser interaction pass. |
...
- 1.2 can close only for static foundations: shared table, shared Kubernetes detail drawer, shared YAML drawer, shared filter toolbar, create/action button primitives, semantic status/chip primitives, and theme tokens exist.
- 1.2 remains open for style normalization: modal family, drawer family widths, page-local table/action exceptions, empty/degraded-state treatment, and mixed inline colors.
