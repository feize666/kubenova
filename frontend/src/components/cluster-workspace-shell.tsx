...
const sectionIcons: Record<string, React.ReactNode> = {
  ...
  observability: <DashboardOutlined />,
...
export function ClusterWorkspaceShell({
  children,
}: {
    ...
    () =>
      navigation
        .filter(
          (section) =>
            section.items.length > 1 &&
            section.items.some((item) => selectedKeys.includes(item.href)),
        )
  ...
  useEffect(() => {
    ...
    window.location.replace(
      buildLoginRoute(buildInternalReturnTo(pathname, search)),
    );
  }, [isAuthenticated, isInitializing, pathname]);

  if (isInitializing)
    return <BootstrapScreen description="正在恢复集群工作区..." />;
  if (!isAuthenticated)
    return <BootstrapScreen description="正在跳转到登录页..." />;
  if (!clusterId)
    return <BootstrapScreen description="集群地址无效，正在返回集群列表..." />;

  const cluster = clusterQuery.data;
  const clusterName =
    cluster?.displayName || cluster?.name || `集群 ${clusterId.slice(0, 8)}`;
  const statusLabel =
    cluster?.runtimeStatus === "running"
      ? "运行中"
      : cluster?.runtimeStatus === "checking"
        ? "探测中"
        : cluster
          ? "离线"
          : "加载中";
  ...
  return (
    <ClusterWorkspaceProvider clusterId={clusterId}>
      <Layout
        className="kubenova-shell cluster-workspace-shell"
      ...
      >
        ...
        <Layout>
          <Header className="app-header cluster-workspace-shell__header">
            ...
            <Breadcrumb
              items={[
                { title: <Link href="/clusters">集群</Link> },
                { title: clusterName },
                { title: currentTitle },
              ]}
            />
            <div
              className="cluster-workspace-header__context"
              aria-label="当前集群上下文"
            >
              ...
              <Typography.Text className="cluster-workspace-header__id">
                {clusterId}
              </Typography.Text>
            </div>
          ...
          <Content
            ...
            className="app-content cluster-workspace-shell__content"
            tabIndex={-1}
          >
            {children}
          </Content>
