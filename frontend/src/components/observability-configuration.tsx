...
export function ObservabilityConfiguration() {
  ...
  useEffect(() => {
    ...
    if (editor.type === "source") {
      form.setFieldsValue(
        editor.record ?? { kind: "prometheus", enabled: true },
      );
    } else if (editor.type === "alert") {
      form.setFieldsValue(
        editor.record ?? { severity: "warning", duration: "5m", enabled: true },
      );
    } else {
      form.setFieldsValue(
        editor.record ?? {
  ...
  const invalidate = useCallback(async () => {
    await Promise.all([
      queryClient.invalidateQueries({
        queryKey: ["observability", "config", "sources"],
      }),
      queryClient.invalidateQueries({
        queryKey: ["observability", "config", "alerts"],
      }),
      queryClient.invalidateQueries({
        queryKey: ["observability", "config", "notifications"],
      }),
    ]);
  }, [queryClient]);
    ...
    mutationFn: async (values: Record<string, unknown>) => {
      ...
      if (editor.type === "source") {
        ...
        return editor.record
          ...
          : createObservabilityDataSource(payload, accessToken || undefined);
      }
      if (editor.type === "alert") {
        return editor.record
          ? updateAlertTemplate(
              editor.record.id,
              values as unknown as Partial<AlertTemplateInput>,
              accessToken || undefined,
            )
          : createAlertTemplate(
              values as unknown as AlertTemplateInput,
              accessToken || undefined,
            );
      }
      return editor.record
  ...
  const remove = useCallback(
    async (type: EditorState["type"], id: string) => {
      if (!canWrite) {
      ...
      try {
        if (type === "source")
          await deleteObservabilityDataSource(id, accessToken || undefined);
        if (type === "alert")
  ...
  const testSource = useCallback(
    async (record: ObservabilityDataSource) => {
      ...
      try {
        const result = record.id.startsWith("env-")
          ...
          : await testObservabilityDataSource(
              record.id,
              accessToken || undefined,
            );
        messageApi[result.status === "healthy" ? "success" : "warning"](
          `${record.name}：${result.status}${result.latencyMs === null ? "" : ` · ${result.latencyMs} ms`}`,
        );
  ...
  const sourceColumns = useMemo<ColumnsType<ObservabilityDataSource>>(
    () => [
      ...
      },
      {
        ...
        dataIndex: "kind",
        key: "kind",
        render: (value: string) => kindLabels[value] ?? value,
      },
      {
        title: "状态",
        dataIndex: "status",
        ...
        render: (_, record) => (
          <Space size={4}>
            <OpsIconActionButton
              ...
              onClick={() => void testSource(record)}
            >
              探针
            </OpsIconActionButton>
            {record.id.startsWith("env-") ? null : (
              <>
                <OpsIconActionButton
                  size="small"
                  disabled={!canWrite}
                  disabledReason={!canWrite ? "当前账号无写入权限" : undefined}
                  icon={<EditOutlined />}
                  onClick={() => setEditor({ type: "source", record })}
                >
                  编辑
                </OpsIconActionButton>
                <OpsIconActionButton
                  ...
                  disabledReason={!canWrite ? "当前账号无写入权限" : undefined}
                  icon={<DeleteOutlined />}
                  onClick={() => void remove("source", record.id)}
                >
                  删除
                </OpsIconActionButton>
              </>
    ...
    () => [
      {
        ...
        render: (value: string, record) => (
          <Space>
            <Typography.Text strong>{value}</Typography.Text>
            {record.enabled ? (
              <OpsStatusTag tone="success">启用</OpsStatusTag>
            ) : (
              <OpsStatusTag tone="neutral">停用</OpsStatusTag>
            )}
          </Space>
        ),
      },
        ...
        render: (_, record) => (
          <Space size={4}>
            <OpsIconActionButton
              ...
              disabledReason={!canWrite ? "当前账号无写入权限" : undefined}
              icon={<EditOutlined />}
              onClick={() => setEditor({ type: "alert", record })}
            >
              编辑
            </OpsIconActionButton>
            <OpsIconActionButton
              ...
              disabledReason={!canWrite ? "当前账号无写入权限" : undefined}
              icon={<DeleteOutlined />}
              onClick={() => void remove("alert", record.id)}
            >
              删除
            </OpsIconActionButton>
          </Space>
        ...
        render: (_, record) => (
          <Space size={4}>
            <OpsIconActionButton
              ...
              disabledReason={!canWrite ? "当前账号无写入权限" : undefined}
              icon={<EditOutlined />}
              onClick={() => setEditor({ type: "notification", record })}
            >
              编辑
            </OpsIconActionButton>
            <OpsIconActionButton
              ...
              disabledReason={!canWrite ? "当前账号无写入权限" : undefined}
              icon={<DeleteOutlined />}
              onClick={() => void remove("notification", record.id)}
            >
              删除
            </OpsIconActionButton>
          </Space>
    ...
    <div className="resource-workbench" style={{ display: "grid", gap: 16 }}>
      ...
      <OpsSurface variant="panel" padding="sm">
        <ResourcePageHeader
          ...
          description={
            clusterId
              ? "配置当前集群的数据源、告警规则与通知策略"
              : "配置平台级可观测性数据源、告警规则与通知策略"
          }
          extra={
            <Space>
              <OpsIconActionButton
      ...
      {!enabled ? (
        <Alert type="warning" showIcon title="请先登录后管理观测配置。" />
      ) : null}
      {enabled && !canWrite ? (
        <Alert
          type="info"
          showIcon
          title="当前账号为只读角色，可查看和探测观测状态；修改配置需要管理员权限。"
        />
      ) : null}
          ...
          items={[
            {
              ...
              children: (
                <>
                  ...
                  <ResourceTable<ObservabilityDataSource>
                    tableKey="observability-sources"
                    showToolbar={false}
            ...
            },
            {
              ...
              children: (
                <>
                  ...
                  >
                    <Typography.Text type="secondary">
                      规则表达式原样保存，后续由 PrometheusRule 渲染器发布。
                    </Typography.Text>
                    <Button
                      type="primary"
                      disabled={!canWrite}
                      icon={<PlusOutlined />}
                      onClick={() => setEditor({ type: "alert" })}
                    >
                      添加告警规则
                    </Button>
                  </div>
                  <ResourceTable<AlertTemplate>
                    tableKey="observability-alerts"
                    showToolbar={false}
                    viewportScroll={false}
                    rowKey="id"
                    size="small"
                ...
                <>
                  ...
                  >
                    ...
                    <Button
                      ...
                      disabled={!canWrite}
                      icon={<PlusOutlined />}
                      onClick={() => setEditor({ type: "notification" })}
                    >
                      添加通知模板
                    </Button>
                  </div>
      ...
      <OpsModalShell
        title={`${editor?.record ? "编辑" : "添加"}${editorTitle ?? "配置"}`}
        description="配置变更会在保存后立即应用到当前集群或平台级观测能力。"
        open={Boolean(editor)}
      ...
      >
        {editor?.type === "source" ? (
          <Form form={form} layout="vertical">
            ...
            >
              <Input placeholder="production-prometheus" />
            </Form.Item>
            <Form.Item name="kind" label="类型" rules={[{ required: true }]}>
              <Select
                options={(
                  catalogQuery.data?.dataSourceKinds ?? [...OBSERVABILITY_KINDS]
                ).map((value) => ({
                  value,
        ...
        {editor?.type === "alert" ? (
          <Form form={form} layout="vertical">
            ...
            >
              <Input placeholder="HighPodRestartRate" />
            </Form.Item>
            <Form.Item
              name="severity"
              label="级别"
            ...
            >
              <Input.TextArea
                rows={4}
                placeholder={
                  "sum(rate(kube_pod_container_status_restarts_total[5m])) > 1"
                }
              />
            </Form.Item>
        ...
        {editor?.type === "notification" ? (
          <Form form={form} layout="vertical">
            ...
            >
              <Input placeholder="平台告警通知" />
            </Form.Item>
            <Form.Item name="channel" label="渠道" rules={[{ required: true }]}>
              <Select
                options={(
                  catalogQuery.data?.notificationChannels ?? [
                    ...NOTIFICATION_CHANNELS,
                  ]
                ).map((value) => ({
                  value,
