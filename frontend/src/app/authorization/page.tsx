...
export default function AuthorizationPage() {
  ...
  return <div className="authorization-page">
    ...
    {tab === "users" ? <UsersPage /> : <section className="authorization-page__empty"><h1>集群访问授权</h1><p>按集群、命名空间和能力范围授予访问权限。</p><div className="authorization-page__capabilities">{capabilities.map((item) => <span key={item}>{item}</span>)}</div>{grants.isLoading ? <p>加载授权中…</p> : grants.data?.items.map((grant) => <article key={grant.id}><strong>{grant.principal?.username ?? grant.principal?.name ?? "未知主体"}</strong> · {grant.cluster.name} · {grant.role}<br /><small>命名空间：{grant.namespaces.map((item) => item.name).join(", ") || "全部"}　能力：{grant.capabilities.join(", ") || "基础查看"}</small></article>)}</section>}
  </div>;
}
