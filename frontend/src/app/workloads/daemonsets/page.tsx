  ...
  const handleScale = async () => {
    ...
    try {
      ...
      void message.success(
        `${selected.name} 扩缩容操作已提交，目标值 ${target}`,
      );
    } catch (err) {
      void message.error(
  ...
  return (
    <Space orientation="vertical" size={16} style={{ width: "100%" }}>
            ...
            {editingItem ? (
              <>
                ...
                <Form.Item label="Annotations" name="annotationsText">
                  <Input.TextArea
                    rows={4}
                    placeholder={"description=node-agent\nowner=team-a"}
                  />
                </Form.Item>
              </>
            ) : null}
      ...
      >
        <OpsFormSection
          title="调度目标"
          description="提交期望调度数量，实际覆盖由集群调度器决定。"
        >
