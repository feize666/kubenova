...
import {
  ...
  filterClusterScopedColumns,
  getClusterIdFromPathname,
  getClusterWorkspaceNavigation,
  isSupportedClusterWorkspaceResource,
  resolveAccordionOpenKeys,
  resolveResourceFilterBasePath,
  resolveWorkspaceClusterFieldValue,
...
test("集群工作区菜单同一时间只展开一个资源域", () => {
  assert.deepEqual(
    resolveAccordionOpenKeys(["base-resources", "workloads"], ["workloads"]),
    ["base-resources"],
  );
  assert.deepEqual(
    resolveAccordionOpenKeys(
      ["workloads", "network", "storage"],
      ["workloads"],
    ),
    ["network"],
  );
  assert.deepEqual(resolveAccordionOpenKeys(["base-resources", "workloads"]), [
    "workloads",
  ]);
  assert.deepEqual(resolveAccordionOpenKeys([]), []);
});

test("集群入口生成固定集群的 canonical 概览地址", () => {
  assert.equal(
    buildClusterWorkspaceHref(" ack-prod "),
    "/clusters/ack-prod/overview",
  );
  assert.equal(
    buildClusterWorkspaceHref("ack/prod"),
    "/clusters/ack%2Fprod/overview",
  );
});
...
test("单集群工作区只保留当前集群资源菜单", () => {
  ...
  assert.deepEqual(
    baseResources?.items.map((item) => item.label),
    ["Node", "Namespace"],
  );
  ...
  assert.ok(
    sections
      ...
      .every((item) => item.href.startsWith("/clusters/ack-prod/")),
  );
});

test("从工作区地址解析并解码唯一集群标识", () => {
  assert.equal(
    getClusterIdFromPathname("/clusters/ack-prod/overview"),
    "ack-prod",
  );
  assert.equal(
    getClusterIdFromPathname("/clusters/ack%2Fprod/workloads/pods"),
    "ack/prod",
  );
...
test("工作区集群标识覆盖旧查询范围且不可清空", () => {
  assert.equal(
    resolveWorkspaceClusterId("ack-prod", "other-cluster"),
    "ack-prod",
  );
  assert.equal(resolveWorkspaceClusterId("ack-prod", ""), "ack-prod");
  assert.equal(
    resolveWorkspaceClusterId(null, "legacy-cluster"),
    "legacy-cluster",
  );
});
...
test("工作区集群表单值始终锁定当前集群", () => {
  assert.equal(
    resolveWorkspaceClusterFieldValue("ack-prod", "other-cluster"),
    "ack-prod",
  );
  assert.equal(resolveWorkspaceClusterFieldValue(" ack-prod ", ""), "ack-prod");
  assert.equal(
    resolveWorkspaceClusterFieldValue(null, "legacy-cluster"),
    "legacy-cluster",
  );
});
...
test("单集群工作区隐藏集群列且不影响 legacy 页面", () => {
  ...
  assert.deepEqual(
    filterClusterScopedColumns("ack-prod", columns).map((column) =>
      "key" in column ? column.key : undefined,
    ),
    ["name", "namespace"],
  );
  assert.deepEqual(
    filterClusterScopedColumns(null, columns).map((column) =>
      "key" in column ? column.key : undefined,
    ),
...
test("单集群工作区递归清理分组中的集群列", () => {
  ...
  assert.deepEqual(filterClusterScopedColumns("ack-prod", columns), [
    {
      key: "identity",
      title: "身份",
      children: [{ key: "name", title: "名称" }],
    },
  ]);
  assert.deepEqual(
    filterClusterScopedColumns("ack-prod", [
      {
        key: "identity",
        title: "身份",
        children: [{ key: "clusterId", title: "集群" }],
      },
    ]),
    [],
  );
