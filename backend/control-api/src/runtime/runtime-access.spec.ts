...
describe('runtime HTTP authorization boundaries', () => {
  ...
  const request = { user: { user: actor }, headers: {} };
  ...
  it('ignores a forged userId and binds the session to the authenticated actor', async () => {
    const { controller, access, service } = setup();
    await controller.createSession({ ...target, userId: 'administrator' }, request as never);
    expect(access.assertCanMutate).toHaveBeenCalledWith(actor, 'cluster-a');
    expect(service.createSession).toHaveBeenCalledWith(expect.objectContaining({ userId: actor.id }), expect.any(Object));
  });
  ...
  it('passes authenticated ownership through log stream bootstrap', async () => {
    ...
    expect(service.createStreamSession).toHaveBeenCalledWith(target, expect.objectContaining({ userId: actor.id }));
  });
});
